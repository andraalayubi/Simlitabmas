...
export async function GET() {
  try {
    ...
    const valid_proposal_suggestion = proposal_suggestions.filter((item: any) => {
      ...
      const penelitian_action = workflow.getAction(item.status, item.phase, 'admin', 'penelitian');
      const pengmas_action = workflow.getAction(item.status, item.phase, 'admin', 'pengmas');
